...
function crawlerInit (cmdResponse) {
	...
	countdown.start();
	maxItem = cmdResponse.items;
	currentItem = cmdResponse.fromItem;
	...
	requestOpts.url = url;
	var realMaxItemPromise	= request(requestOpts).then(function ($) {
		var realMaxItem = $('div.text-center.ak-pagination.hidden-xs').find('ul.ak-pagination.pagination.ak-ajaxloader li:last-child').prev().prev().text().trim();
		return realMaxItem;
	});
    console.log("TOTO");
    ...
    function(realMaxPage) {
		...
		if (realMaxPage >= maxPage && currentPage <= maxPage) {
			var callback =  function(values) {
                itemslinks.push(values);
				currentPage++;
		        if(currentPage <= maxPage) {
		         	getPageLinks(currentPage, callback); 
		        }else {
                    itemslinks = concatToOneArray(itemslinks);
